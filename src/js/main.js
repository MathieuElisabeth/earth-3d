...
let checkboxBackground = document.querySelector("#switch-button");
...
(async function() {
  ...
  let envmapTexture = await new RGBELoader()
    ...
    .loadAsync("src/assets/old_room_2k.hdr");  // thanks to https://polyhaven.com/hdris !
  ...
  let textures = {
    ...
    bump: await new THREE.TextureLoader().loadAsync("src/assets/earthbump.jpg"),
    map: await new THREE.TextureLoader().loadAsync("src/assets/earthmap.jpg"),
    spec: await new THREE.TextureLoader().loadAsync("src/assets/earthspec.jpg"),
    planeTrailMask: await new THREE.TextureLoader().loadAsync("src/assets/mask.png"),
  };

  // "Cartoon Plane" (https://skfb.ly/UOLT) by antonmoek
  let plane = (await new GLTFLoader().loadAsync("src/assets/plane/scene.glb")).scene.children[0]
  let planesData = [
  ...
  if (daytime && checkboxBackground.checked) {
    checkboxBackground.checked = false
  }


  checkboxBackground.addEventListener("click", (e) => {
    if(animating) return;

    let anim;
    if(!checkboxBackground.checked) {
      anim = [1, 0];
    } else if(checkboxBackground.checked) {
      anim = [0, 1];
    } else {
      return;
    }
